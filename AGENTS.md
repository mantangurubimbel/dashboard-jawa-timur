...
All user-facing static text must be written in English, including:
- Navigation menus
- Sidebar menus
- Page titles
...
- Dropdown options generated by the application
- Modal titles
...
- Footer notes
- Informational and help text

### Database / API Data
Do **not translate, modify, normalize, or replace data retrieved from the database or API**.

Database/API values must be displayed in their original form unless the task explicitly requires presentation formatting.

Examples:
- Student names must remain unchanged.
- School names must remain unchanged.
- City/region names must remain unchanged.
- Product names must remain unchanged.
- Address data must remain unchanged.
- User-entered text must remain unchanged.

### Mixed Language Rule
Use English for the application interface and preserve the original language of source data.

Examples:
- Table header: `School Name`
- Table data: `SMA Negeri 1 Surabaya`
- Button: `Add Student`
- Database value: `Jawa Timur`

### No Unrequested Localization Changes
Do not translate existing database values, API responses, or user-generated content.

Do not perform broad localization or translation refactors unless explicitly requested.

When modifying an existing component, convert only static UI text to English where necessary while preserving source data exactly.

### Developer / Code Language
Code comments, variable names, function names, component names, and new documentation should preferably use English.
...
## 9. File and code-change discipline
...
## 16. Prompt/task scoping guidance for Codex
