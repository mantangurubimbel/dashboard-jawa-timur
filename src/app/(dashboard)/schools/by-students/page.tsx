...
import { StudentSchoolPartnerTable } from "@/components/student-school-partner-table";
...
export default async function StudentsPage({
  ...
  return (
    <div className="flex w-full flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
      <header className="border-b border-slate-200 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="mt-1 text-3xl font-semibold text-slate-950">By BAC Students</h1>
          </div>
        </div>
        <p className="mt-2 text-sm text-slate-600">
          Summary of BAC students, status, branch distribution, and registration trends for academic year {academicYear || "-"}.
        </p>
      </header>
      <StudentFilters
      ...
      <StudentSchoolPartnerTable rows={data.schoolPartner} />
    </div>
  );
}
