...
export default async function SettingsPage() {
  await requireAdmin();
  const [users, branches, roles, userRows, positionRows, userBranchRows, branchRows] = await Promise.all([
    fetchCount("t_app_user"),
    fetchCount("t_branch"),
    fetchCount("t_role"),
    supabaseRestFetch("t_app_user?select=id,name,position_id,access_revenue_dashboard&user_grade=gte.2&order=name&limit=1000"),
  ...
  const profiles = userRows.ok ? await userRows.json() as { id: string; name?: string; position_id?: number; access_revenue_dashboard?: boolean }[] : [];
  ...
  const adminUsers = profiles.map((profile) => ({ id: profile.id, name: profile.name ?? "", position: positionsMap.get(profile.position_id ?? -1) ?? "", branches: branchesByUser.get(profile.id) ?? [], accessRevenue: Boolean(profile.access_revenue_dashboard) }));
    ...
    <div className="flex w-full flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">Upload raw transactions and replace data by date.</p>
