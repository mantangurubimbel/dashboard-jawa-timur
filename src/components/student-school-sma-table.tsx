  ...
  return (
    <section className="overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 px-4 py-3">
        <h2 className="text-base font-semibold text-slate-950">Jumlah Siswa per Asal Sekolah (SMA)</h2>
        <p className="mt-1 text-sm text-slate-500">Perbandingan YoY dengan LY dan L2Y jenjang SMA</p>
      </div>
      <div className="max-h-[760px] overflow-auto">
        <table className="min-w-[980px] table-fixed border-collapse text-left text-sm">
          ...
          <tbody>
            ...
            {!rows.length ? (
              <tr><td colSpan={13} className="px-3 py-8 text-center text-slate-500">Belum ada data sekolah SMA.</td></tr>
            ) : null}
          </tbody>
        </table>
      </div>
      <div className="border-t border-slate-200 px-4 py-2 text-xs text-slate-500">
        {formatNumber(rows.length)} sekolah ditampilkan. Gapyear masuk perhitungan 12 SMA
      </div>
    </section>
  );
}
