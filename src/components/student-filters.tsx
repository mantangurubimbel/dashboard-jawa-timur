    ...
    <section className="border-y border-slate-200 bg-white">
      <div className="flex w-full flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-slate-950">Filter Student</span>
        </div>
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          {showDateFilters ? <><label className="flex items-center gap-1.5"><span className="text-xs text-slate-500">Dari tanggal</span><input type="date" value={draft.fromDate} onChange={(event) => update("fromDate", event.target.value)} className="h-8 w-32 rounded-md border border-slate-300 bg-white px-2 text-xs text-slate-800" /></label><label className="flex items-center gap-1.5"><span className="text-xs text-slate-500">Sampai tanggal</span><input type="date" value={draft.toDate} onChange={(event) => update("toDate", event.target.value)} className="h-8 w-32 rounded-md border border-slate-300 bg-white px-2 text-xs text-slate-800" /></label></> : null}
          <button
            type="button"
            onClick={reset}
            disabled={pending}
